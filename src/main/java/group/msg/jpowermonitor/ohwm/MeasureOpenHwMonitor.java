...
public class MeasureOpenHwMonitor implements MeasureMethod {
    ...
    public void init(String configFile) {
        config = JPowerMonitorConfig.readConfig(configFile);
    ...
    public @NotNull List<String> configuredSensors() {
        return config.getOpenHardwareMonitor().getPaths().stream().map(p -> String.join("->", p.getPath())).collect(Collectors.toList());
    }
